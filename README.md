...
| seals_locked | int | Selos bloqueados pelo solicitante ao pedir entrada acima do limite gratuito (padrão: 0) |
| created_at | timestamptz | — |
...
| `count_user_memberships(user_id)` | Retorna int — conta grupos ativos em que o usuário é membro mas não dono (usado para aplicar limite de participação) |
...
| `group_join_fee` | Entrada em Grupo | — (custo de 30 selos para entrar a partir do 3º grupo como membro) |
